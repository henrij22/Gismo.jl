...
mutable struct KnotVector
    ...
    function KnotVector(a::Vector{Float64})
        kv = new(ccall((:gsKnotVector_create, libgismo), Ptr{gsCKnotVector},
            (Ptr{Cdouble}, Cint), a, length(a)))
...
function Base.show(io::IO, obj::KnotVector)
    ccall((:gsKnotVector_print, libgismo), Cvoid, (Ptr{gsCKnotVector},), obj.ptr)
end
...
function BSplineBasis(kv::KnotVector)::Basis
    b = ccall(
        (:gsBSplineBasis_create, libgismo), Ptr{gsCBasis}, (Ptr{gsCKnotVector},), kv.ptr)
    return Basis(b)
end
...
function TensorBSplineBasis(kv::Vararg{KnotVector})::Basis
    d = Base.length(kv)
    if (d==1)
        b = ccall((:gsBSplineBasis_create, libgismo),
            Ptr{gsCBasis}, (Ptr{gsCKnotVector},), kv[1].ptr)
    elseif (d==2)
        b = ccall((:gsTensorBSplineBasis2_create, libgismo), Ptr{gsCBasis},
            (Ptr{gsCKnotVector}, Ptr{gsCKnotVector}), kv[1].ptr, kv[2].ptr)
    elseif (d==3)
        b = ccall((:gsTensorBSplineBasis3_create, libgismo), Ptr{gsCBasis},
            (Ptr{gsCKnotVector},
                Ptr{gsCKnotVector},
                Ptr{gsCKnotVector}),
            kv[1].ptr,
            ...
            kv[3].ptr)
    elseif (d==4)
        b = ccall((:gsTensorBSplineBasis4_create, libgismo),
            Ptr{gsCBasis},
            (Ptr{gsCKnotVector},
                Ptr{gsCKnotVector},
                Ptr{gsCKnotVector},
...
function knots(basis::Basis, dir::Int)::KnotVector
    if (domainDim(basis)==1)
        kv = ccall((:gsBSplineBasis_knots, libgismo), Ptr{gsCKnotVector},
            (Ptr{gsCBasis}, Cint), basis.ptr, dir)
    else
        kv = ccall((:gsTensorBSplineBasis_knots, libgismo),
            Ptr{gsCKnotVector}, (Ptr{gsCBasis}, Cint), basis.ptr, dir)
    end
...
function NurbsBasis(kv::KnotVector)::Basis
    b = ccall(
        (:gsNurbsBasis_create, libgismo), Ptr{gsCBasis}, (Ptr{gsCKnotVector},), kv.ptr)
    return Basis(b)
end
...
function TensorNurbsBasis(kv::Vararg{KnotVector})::Basis
    d = Base.length(kv)
    if (d==1)
        b = ccall((:gsNurbsBasis_create, libgismo), Ptr{gsCBasis},
            (Ptr{gsCKnotVector},), kv[1].ptr)
    elseif (d==2)
        b = ccall((:gsTensorNurbsBasis2_create, libgismo), Ptr{gsCBasis},
            (Ptr{gsCKnotVector}, Ptr{gsCKnotVector}), kv[1].ptr, kv[2].ptr)
    elseif (d==3)
        b = ccall((:gsTensorNurbsBasis3_create, libgismo), Ptr{gsCBasis},
            (Ptr{gsCKnotVector},
                Ptr{gsCKnotVector},
                Ptr{gsCKnotVector}),
            kv[1].ptr,
            ...
            kv[3].ptr)
    elseif (d==4)
        b = ccall((:gsTensorNurbsBasis4_create, libgismo),
            Ptr{gsCBasis},
            (Ptr{gsCKnotVector},
                Ptr{gsCKnotVector},
                Ptr{gsCKnotVector},
...
function BSplineUnitInterval(deg::Int)::Geometry
    g = ccall(
        (:gsNurbsCreator_BSplineUnitInterval, libgismo), Ptr{gsCGeometry}, (Cint,), deg)
    return Geometry(g)
end
...
function BSplineRectangle(low_x::Cdouble = 0.0,
    ...
    g = ccall((:gsNurbsCreator_BSplineRectangle, libgismo), Ptr{gsCGeometry},
        (Cdouble, Cdouble, Cdouble, Cdouble, Cdouble), low_x, low_y, upp_x, upp_y, turndeg)
    return Geometry(g)
end
...
function BSplineTrapezium(Lbot::Cdouble = 1.0,
        ...
        d::Cdouble = 0.0,
        turndeg::Cdouble = 0.0)::Geometry
    g = ccall((:gsNurbsCreator_BSplineTrapezium, libgismo), Ptr{gsCGeometry},
        (Cdouble, Cdouble, Cdouble, Cdouble, Cdouble), Lbot, Ltop, H, d, turndeg)
    return Geometry(g)
end
...
function BSplineSquare(r::Cdouble = 1.0,
    ...
    g = ccall((:gsNurbsCreator_BSplineSquare, libgismo),
        Ptr{gsCGeometry}, (Cdouble, Cdouble, Cdouble), r, x, y)
    return Geometry(g)
end
...
function BSplineSquareGrid(n::Int,
    ...
    g = ccall((:gsNurbsCreator_BSplineSquareGrid, libgismo), Ptr{gsCMultiPatch},
        (Cint, Cint, Cdouble, Cdouble, Cdouble), n, m, r, lx, ly)
    return MultiPatch(g)
end
...
function BSplineCube(r::Cdouble = 1.0,
    ...
    g = ccall((:gsNurbsCreator_BSplineCube, libgismo), Ptr{gsCGeometry},
        (Cdouble, Cdouble, Cdouble, Cdouble), r, x, y, z)
    return Geometry(g)
end
...
function BSplineCubeGrid(n::Int,
    ...
    g = ccall((:gsNurbsCreator_BSplineCubeGrid, libgismo), Ptr{gsCMultiPatch},
        (Cint, Cint, Cint, Cdouble, Cdouble, Cdouble, Cdouble), n, m, p, r, lx, ly, lz)
    return MultiPatch(g)
end
...
function NurbsQuarterAnnulus(r1::Cdouble = 1.0,
    ...
    g = ccall((:gsNurbsCreator_NurbsQuarterAnnulus, libgismo),
        Ptr{gsCGeometry}, (Cdouble, Cdouble), r1, r2)
    return Geometry(g)
end
...
function NurbsAnnulus(r1::Cdouble = 1.0,
        r2::Cdouble = 2.0)::Geometry
    return ccall((:gsNurbsCreator_NurbsAnnulus, libgismo),
        Ptr{gsCGeometry}, (Cdouble, Cdouble), r1, r2) |> Geometry
end
...
function NurbsSphere(r::Cdouble = 1.0,
        ...
        y::Cdouble = 0.0,
        z::Cdouble = 0.0)::Geometry
    g = ccall((:gsNurbsCreator_NurbsSphere, libgismo), Ptr{gsCGeometry},
        (Cdouble, Cdouble, Cdouble, Cdouble), r, x, y, z)
    return Geometry(g)
end
...
function NurbsCircle(r::Cdouble = 1.0,
        x::Cdouble = 0.0,
        y::Cdouble = 0.0)::Geometry
    g = ccall((:gsNurbsCreator_NurbsCircle, libgismo),
        Ptr{gsCGeometry}, (Cdouble, Cdouble, Cdouble), r, x, y)
    return Geometry(g)
end
...
function BSplineTriangle(H::Cdouble = 1.0,
    ...
    g = ccall((:gsNurbsCreator_BSplineTriangle, libgismo),
        Ptr{gsCGeometry}, (Cdouble, Cdouble), H, W)
    return Geometry(g)
end
...
function BSplineStar(N::Int = 3,
    ...
    g = ccall((:gsNurbsCreator_BSplineStar, libgismo),
        Ptr{gsCMultiPatch}, (Cint, Cdouble, Cdouble), N, R0, R1)
    return MultiPatch(g)
end
