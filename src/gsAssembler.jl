...
mutable struct QuadRule
    ...
    function QuadRule(quad::Ptr{gsCQuadRule}, delete::Bool = true)
        ...
        return b
    end#= This function depends on Basis and OptionList, which are dependencies. Can we do some forward declarations, or should we take care of this in the ordering in Declaraions.jl? =#
...
function GaussRule(d::Cint, numNodes::Array{Cint}, digits::Cint = Cint(0))::QuadRule
    ...
    qr = ccall((:gsGaussRule_create, libgismo), Ptr{gsCQuadRule},
        (Cint, Ptr{Cint}, Cint), d, numNodes, digits)
    return QuadRule(qr)
end
...
function GaussRule(numNodes::Cint, digits::Cint = Cint(0))::QuadRule
    ...
    qr = ccall((:gsGaussRule_create, libgismo), Ptr{gsCQuadRule},
        (Cint, Ptr{Cint}, Cint), d, numNodesVec, digits)
    return QuadRule(qr)
end
...
function LobattoRule(d::Cint, numNodes::Array{Cint}, digits::Cint = Cint(0))::QuadRule
    ...
    qr = ccall((:gsLobattoRule_create, libgismo), Ptr{gsCQuadRule},
        (Cint, Ptr{Cint}, Cint), d, numNodes, digits)
    return QuadRule(qr)
end
...
function LobattoRule(numNodes::Int, digits::Int = Cint(0))::QuadRule
    ...
    qr = ccall((:LobattoRule_create, libgismo), Ptr{gsCQuadRule},
        (Cint, Ptr{Cint}, Cint), d, numNodesVec, digits)
    return QuadRule(qr)
end
...
"""
function mapTo(qr::QuadRule, lower::Vector{Cdouble},
        upper::Vector{Cdouble})::Tuple{EigenMatrix, EigenMatrix}
...
"""
function mapTo(
        qr::QuadRule, startVal::Cdouble, endVal::Cdouble)::Tuple{EigenMatrix, EigenMatrix}
